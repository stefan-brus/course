...
character = P pf
  where
    pf Nil = ErrorResult UnexpectedEof
    pf (c :. cs) = Result cs c
...
(>>>) ::
  ...
  -> Parser b
  -> Parser b
p1 >>> p2 = bindParser (\_ -> P (\i -> parse p2 i)) p1
...
list ::
  Parser a
  -> Parser (List a)
list p = P pf ||| valueParser Nil
  where
    pf i = case parse p i of
      ErrorResult e -> ErrorResult e
      Result i' x -> parse (mapParser (x :.) $ list p) i'
...
list1 p = P pf
  where
    pf Nil = ErrorResult UnexpectedEof
    pf i = parse (bindParser valueParser $ list p) i
...
satisfy f = P pf
  where
    ...
    pf i@(c :. cs)
      | f c = Result cs c
      | otherwise = parse (bindParser unexpectedCharParser character) i
...
natural = P pf
  where
    pf Nil = ErrorResult UnexpectedEof
    pf i@(c :. _) = case parse (mapParser (read :: Chars -> Optional Int) $ list1 digit) i of
      ErrorResult e -> ErrorResult e
      Result _ Empty -> ErrorResult (UnexpectedChar c)
      Result i' (Full x) -> Result i' x
...
sequenceParser (p :. ps) = P pf
  where
    pf i = case parse p i of
      ErrorResult e -> ErrorResult e
      Result i' x -> parse (mapParser (x :.) $ sequenceParser ps) i'
...
firstNameParser ::
  Parser Chars
firstNameParser = bindParser (\x -> mapParser (x :.) $ list lower) upper
...
surnameParser ::
  Parser Chars
surnameParser = flbindParser (bindParser (\x -> mapParser (x :.) $ thisMany 5 lower) upper) $ (\x -> mapParser (x ++) $ list lower)
...
phoneParser ::
  Parser Chars
phoneParser = flbindParser digit $ (\x -> mapParser (x :.) $ flbindParser phoneBodyParser (\y -> mapParser (y ++) (is '#' >>> valueParser Nil)))
...
instance Apply Parser where
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  p1 <*> p2 = flbindParser p1 (\f -> mapParser f p2)
