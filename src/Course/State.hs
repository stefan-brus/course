...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  f <$> x = State (\s -> let (v, s') = runState x s in ((f v), s'))
...
instance Apply (State s) where
  (<*>) ::
    ...
    -> State s a
    -> State s b
  a <*> x = State (\s -> let (f, s') = runState a s in runState (f <$> x) s')
...
instance Bind (State s) where
  (=<<) ::
    ...
    -> State s a
    -> State s b
  f =<< x = State (\s -> let (v, s') = runState x s in runState (f v) s')
...
findM f (x :. xs) = do
  b <- f x
  if b then return (Full x) else findM f xs
...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat = flip eval S.empty . findM found
  where
    found :: (Ord a) => a -> State (S.Set a) Bool
    found x = do
      st <- get
      if S.member x st
      then pure True
...
distinct ::
  ...
  List a
  -> List a
distinct = flip eval S.empty . filtering found
  where
    found :: (Ord a) => a -> State (S.Set a) Bool
    found x = do
      st <- get
...
isHappy ::
  Integer
  -> Bool
isHappy = contains 1 . flip eval S.empty . findM found . produce sumDigSq
  where
    found :: Integer -> State (S.Set Integer) Bool
    found x = if x == 1 then pure True else do
      st <- get
      if S.member x st
      then pure True
      else do
    ...
    sumDigSq :: Integer -> Integer
    sumDigSq = toInteger . sum . map (join (*)) . map digitToInt . show'
